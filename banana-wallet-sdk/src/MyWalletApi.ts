...
import {
  BananaAccount__factory,
  BananaAccountProxyFactory__factory,
...
export class MyWalletApi extends SimpleAccountAPI {
  ...
  async _getAccountContract(): Promise<BananaAccount> {
    if (this.accountContract == null) {
      this.accountContract = BananaAccount__factory.connect(
  ...
  getTouchIdSafeWalletContractInitializer = (): string => {
    const TouchIdSafeWalletContractSingleton: BananaAccount =
      BananaAccount__factory.connect(
        this.singletonTouchIdSafeAddress,
        this.provider
      );
    const TouchIdSafeWalletContractQValuesArray: Array<string> = [
      this.qValues[0],
      this.qValues[1],
    ];
    //@ts-ignore
    const TouchIdSafeWalletContractInitializer =
      TouchIdSafeWalletContractSingleton.interface.encodeFunctionData(
        "setupWithEntrypoint",
        [
  ...
  async getAccountInitCode(): Promise<string> {
    if (this.factory == null) {
      if (this.factoryAddress != null && this.factoryAddress !== "") {
        this.factory = BananaAccountProxyFactory__factory.connect(
          this.factoryAddress,
          this.provider
        );
      } else {
  ...
  async encodeExecute(
    ...
    return accountContract.interface.encodeFunctionData(
      ...
      [target, value, data, delegateCall]
    );
  }
  ...
  async encodeUserOpCallDataAndGasLimitForBatchedTransaction(
